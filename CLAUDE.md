...
```
Production десктоп ходит на HTTPS-бэкенд `https://83-222-21-227.nip.io` (см. `frontend/.env.tauri`).
...
- Backend для десктопа — `https://83-222-21-227.nip.io` (Let's Encrypt + nginx + nip.io DNS wildcard); web и desktop ходят на один и тот же бэкенд
